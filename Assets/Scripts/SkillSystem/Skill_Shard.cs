...
public class Skill_Shard : Skill_Base
{
    ...
    [SerializeField] private float savedHealPercent;
    ...
    private void HandleShardHealthRewind()
    {
        ...
        {
            CreateShard();
            savedHealPercent = playerHealth.GetHealthPercent();
        }
        else
        {
            SwapPlayerAndShard();
            playerHealth.SetHealthToPercent(savedHealPercent);
            SetSkillOnCoolDown();
        }
    }
    ...
    private void HandleShardMulticase()
    {
        if (currentCharges <= 0)
            return;
