...
public class UI_Quest : MonoBehaviour, ISaveable
{
    ...
    public void SetupQuestUI(QuestDataSO[] questSlots)
    {
        foreach (var slot in this.questSlots)
            slot.gameObject.SetActive(false);

        for (int i = 0; i < questSlots.Length; i++)
        {
            this.questSlots[i].gameObject.SetActive(true);
            this.questSlots[i].SetupQuestSlot(questSlots[i]);
        }
